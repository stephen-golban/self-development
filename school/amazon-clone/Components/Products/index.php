...
if (isset($_GET['category'])) {
    $query = $_GET['category'];
    if (isset($_GET['sortby']) && $_GET['sortby'] == "amazonProd") {
        foreach (DB::getInstance()->getSorted('products', 'WHERE productCategory =', $query, 'productName', 'ASC') as $p) {
            $products[] = $p;
        }
    } else if (isset($_GET['sortby']) && $_GET['sortby'] == "priceCresc") {
        foreach (DB::getInstance()->getSorted('products', 'WHERE productCategory =', $query, 'productPrice', 'ASC') as $p) {
            $products[] = $p;
        }
    } else if (isset($_GET['sortby']) && $_GET['sortby'] == "priceDesc") {
        foreach (DB::getInstance()->getSorted('products', 'WHERE productCategory =', $query, 'productPrice', 'DESC') as $p) {
            $products[] = $p;
        }
    } else if (isset($_GET['sortby']) && $_GET['sortby'] == "rating") {
        foreach (DB::getInstance()->getSorted('products', 'WHERE productCategory =', $query, 'productRating', 'DESC') as $p) {
            $products[] = $p;
        }
...
<body>
    ...
    <div class="products__container">
        <div class="sidebar__left">
            <p class="display-4" style="font-size: 25px;">Show results for</p>
            <?php foreach ($categories as $categ) { ?>
                <a href="index.php?category=<?php echo $categ['categoryName']; ?>" style="<?php if (isset($_GET['category']) && ($_GET['category'] == $categ['categoryName'])) {
                                                                                                echo "color: orange;";
                                                                                            } ?>"><?php echo $categ['categoryName']; ?></a>
            <?php } ?>
            <p class="display-4" style="font-size: 25px;">Sort by</p>
            <div class="custom-checkbox">
                <a href="index.php?category=<?php echo $_GET['category']; ?>&sortby=amazonProd" for="check1">
                    <input type="checkbox" id="check1" <?php if (isset($_GET['sortby']) && $_GET['sortby'] == "amazonProd") : ?> checked<?php endif; ?>>
                    <span>Amazon Basics</span>
                </a>
            </div>
            <div class="custom-checkbox">
                <a href="index.php?category=<?php echo $_GET['category']; ?>&sortby=priceCresc" for="check2">
                    <input type="checkbox" id="check2" <?php if (isset($_GET['sortby']) && $_GET['sortby'] == "priceCresc") : ?> checked<?php endif; ?>>
                    <span>Price: Low to High</span>
                </a>
            </div>
            <div class="custom-checkbox">
                <a href="index.php?category=<?php echo $_GET['category']; ?>&sortby=priceDesc" for="check3">
                    <input type="checkbox" id="check3" <?php if (isset($_GET['sortby']) && $_GET['sortby'] == "priceDesc") : ?> checked<?php endif; ?>>
                    <span>Price: High to Low</span>
                </a>
            </div>
            <div class="custom-checkbox">
                <a href="index.php?category=<?php echo $_GET['category']; ?>&sortby=rating" for="check4">
                    <input type="checkbox" id="check4" <?php if (isset($_GET['sortby']) && $_GET['sortby'] == "rating") : ?> checked<?php endif; ?>>
                    <span>Rating</span>
                </a>
            </div>
        </div>
